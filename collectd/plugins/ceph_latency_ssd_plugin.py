    ...
    def get_stats(self):
        ...
        try:
            output = subprocess.check_output(
              "timeout 30s rados --cluster "+ self.cluster +" -p rbdssd bench 10 write -t 1 -b 65536 2>/dev/null | grep -i latency | awk '{print 1000*$3}'", shell=True)
        ...
        data[ceph_cluster]['cluster']['ssd_avg_latency'] = results[0]
        data[ceph_cluster]['cluster']['ssd_stddev_latency'] = results[1]
        data[ceph_cluster]['cluster']['ssd_max_latency'] = results[2]
        data[ceph_cluster]['cluster']['ssd_min_latency'] = results[3]
